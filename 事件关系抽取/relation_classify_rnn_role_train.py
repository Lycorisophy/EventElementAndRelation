...
from models.rnn_role_model import MyModel
import datetime
...
# 设置全局环境
try:
    args = set_args('config/relation_classify_rnn_role_args.txt')
...
def mymodel_train(args, logger, train_dataloader, validation_dataloader):
    ...
    for _ in trange(epochs, desc='Epochs'):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            input_ids1, input_ids2, labels, input_role1, input_role2 = batch
            input_ids1 = input_ids1.squeeze(1).long()
            input_ids2 = input_ids2.squeeze(1).long()
            loss, tmp_eval_accuracy = model(input_ids1, input_ids2, labels, input_role1, input_role2)
            optimizer.zero_grad()
            loss.backward()
...
def mymodel_test(logger, test_dataloader, the_time=my_time):
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        input_ids2 = input_ids2.squeeze(1).long()
        with torch.no_grad():
            tmp_eval_accuracy = model.test(input_ids1, input_ids2, labels, input_role1, input_role2)
        eval_accuracy += tmp_eval_accuracy
        nb_eval_steps += 1
    try:
        logger.info('准确率为：{:.2f}%'
...
def mymodel_cal(logger, test_dataloader, the_time=my_time):
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        input_ids2 = input_ids2.squeeze(1).long()
        with torch.no_grad():
            pred = model.get_guess(input_ids1, input_ids2, input_role1, input_role2)
        size = pred.size()[0]
        for i in range(size):
            try:
