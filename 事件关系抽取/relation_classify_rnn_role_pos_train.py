...
from models.rnn_role_pos_model import MyModel
import datetime
...
def set_args(filename):
    ...
    parser.add_argument("--mymodel_save_dir",
                        default='checkpoint/relation_classify1d/',
                        type=str)
    parser.add_argument("--learning_rate",
                        default=5e-4,
                        type=float)
    parser.add_argument("--model_name",
                        default='rnn_role_pos_model.bin',
                        type=str,
...
# 设置全局环境
try:
    args = set_args('config/relation_classify_rnn_role_pos_args.txt')
...
def mymodel_train(args, logger, train_dataloader, validation_dataloader):
    ...
    for _ in trange(epochs, desc='Epochs'):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            input1, input2, labels, input_role1, input_role2 = batch
            input1 = input1.permute(1, 0, 2)
            input2 = input2.permute(1, 0, 2)
            loss, tmp_eval_accuracy = model(input1[0].long(), input2[0].long(), labels, input1[1].long(),
                                            input1[1].long(), input1[2].long(), input2[2].long(),
                                            input_role1, input_role2)
            optimizer.zero_grad()
            loss.backward()
...
def get_dataloader(filenames):
    ...
    try:
        ...
        from data.get_relation_from_xml import get_all_rel_and_role
        data = get_all_rel_and_role('data/CEC', tokenizer)
        E1 = data[0]
        E2 = data[1]
        B1 = data[2]
        B2 = data[3]
    ...
    train_input1, validation_input1, train_input2, validation_input2, \
        ...
        train_test_split(input1, input2, labels, input_role1, input_role2,
    ...
    train_data = TensorDataset(train_input1, train_input2, train_labels, inputs_role1, inputs_role2)
