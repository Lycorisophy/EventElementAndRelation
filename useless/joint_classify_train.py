...
from models.joint_model import SuperJointModel as MyModel
import datetime
...
def set_args(filename):
    ...
    parser.add_argument("--mymodel_save_dir",
                        default='checkpoint/joint_classify1/',
                        type=str)
    parser.add_argument("--learning_rate",
                        default=2e-5,
                        type=float)
    parser.add_argument("--model_name",
                        default='joint_model.bin',
                        type=str,
    ...
    parser.add_argument("--mymodel_config_dir",
                        default='config/joint_classify_config.json',
                        type=str)
...
def mymodel_train(args, logger, train_dataloader, test_dataloader):
    ...
    for _ in trange(epochs, desc='Epochs'):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            res1, res2, res3 = model(input1[0].long(), input2[0].long(), labels, input1[1].long(),
                                     input1[1].long(), input1[2].long(), input2[2].long(),
                                     input_ft1, input_ft2)
            # TODO
            loss, tmp_train_accuracy = res3
            loss.backward()
