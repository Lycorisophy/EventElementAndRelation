...
from models.element_model import BbElementEmbedding as MyEmbedding
from models.element_model import MyElectraEmbeddings as ElementEmbedding
from relation_models.base_model import BaseModel as MyModel
from models.ner_model import SuperNerModel as NerModel
import datetime
...
def mymodel_train(args, logger, train_dataloader, test_dataloader):
    ...
    model = MyModel(config=config, args=args)
    nerembedding = MyEmbedding(config=config, args=args)
    nermodel = NerModel(config=config, args=args)
    ee_embedding = ElementEmbedding(config=config)
    if args.fp16:
        embedding.half()
        model.half()
        nermodel.half()
    ...
    for _ in trange(epochs, desc='Epochs'):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            embeded1 = embedding(input1[0].long(), input1[1].long(), input1[2].long(), input1[3].long())
            embeded2 = embedding(input2[0].long(), input2[1].long(), input2[2].long(), input2[3].long())
            cembeded1 = embedding(ft1[0].long(), ft1[1].long(), ft1[2].long(), ft1[3].long())
            cembeded2 = embedding(ft2[0].long(), ft2[1].long(), ft2[2].long(), ft2[3].long())
            embeded1 += cembeded1 / 2
            embeded2 += cembeded2 / 2
